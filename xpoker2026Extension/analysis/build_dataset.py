...
def build_analysis_dataset(
    ...
    with open(log_path, "r") as f:
        for line in f:
            ...
            opponent_holes_all = []
            import re
            _hole_re = re.compile(r'^player(\d+)_hole$')
            for key, value in hidden.items():
                m = _hole_re.match(key)
                if m:
                    player_num = int(m.group(1))
                    if player_num != player_index:
                        opponent_holes_all.append((player_num, value))
            if opponent_holes_all:
                opponent_holes_all.sort(key=lambda x: x[0])
                opponent_hole = opponent_holes_all[0][1]
                if len(opponent_holes_all) > 1:
                    record["_multi_opponent"] = True
                    record["_all_opponent_holes"] = {
                        f"player{pn}": h for pn, h in opponent_holes_all
                    }
