...
def get_street_name(state: "State") -> str:
    ...
    if board_count == 0:
        ...
        import warnings
        warnings.warn(
            f"Unexpected board card count: {board_count}. Defaulting to PREFLOP.",
            UserWarning,
        )
        return "PREFLOP"
...
def get_street_index(state: "State") -> int:
    ...
    if board_count == 0:
        ...
        import warnings
        warnings.warn(
            f"Unexpected board_count={board_count} in get_street_index, "
            "returning 0 (PREFLOP) to stay consistent with get_street_name"
        )
        return 0
...
def build_observation(
    ...
    hero_hole = []
    if state.hole_cards and player_index >= 0 and len(state.hole_cards) > player_index:
