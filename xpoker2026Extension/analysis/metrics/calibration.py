...
def compute_kl_divergence(
    ...
    p_vec = np.array([max(p.get(k, 0.0), eps) for k in keys])
    q_vec = np.array([max(q.get(k, 0.0), eps) for k in keys])
    p_vec = p_vec / p_vec.sum()
    q_vec = q_vec / q_vec.sum()
    
    kl = float(np.sum(p_vec * np.log(p_vec / q_vec)))
...
def compute_js_divergence(
    ...
    NOTE: This returns the JS *divergence* in [0, ln(2)].
    scipy.spatial.distance.jensenshannon returns the JS *distance* = sqrt(JS_div)
    in [0, 1]. When comparing values across modules, use compute_js_distance()
    for consistency with the analysis pipeline.
...
def compute_js_distance(
    ...
    Matches the scale of scipy.spatial.distance.jensenshannon (range [0, 1]).
    Use this for consistency with analyze_beliefs.py and the paper figures.
    ...
    Returns:
        JS distance in [0, 1]
    """
    return float(np.sqrt(compute_js_divergence(p, q, eps)))
