...
def _evaluate_showdown(hero_hole: tuple[str, str], villain_hole: tuple[str, str], board: list[str]) -> int:
    """
    Compare two hands at showdown. Returns >0 if hero wins, <0 if villain wins, 0 for tie.
    Uses PokerKit's StandardHighHand evaluator when available, falls back to heuristic.
    """
    if _POKERKIT_EVAL:
        hero_hand = StandardHighHand.from_game("".join(hero_hole), "".join(board))
        villain_hand = StandardHighHand.from_game("".join(villain_hole), "".join(board))
...
class MonteCarloQValue:
    ...
    def compute_q_values(
        ...
        if legal_actions is None:
            import warnings
            warnings.warn(
                "legal_actions not provided; defaulting to all ActionType values. "
                "Q-values for illegal actions may be included.",
                UserWarning,
                stacklevel=2,
            )
    ...
    def _single_rollout(
        ...
        if cards_needed > 0:
            if deck_idx + cards_needed > len(remaining_deck):
                return 0.0
            for _ in range(cards_needed):
                board.append(remaining_deck[deck_idx])
                deck_idx += 1
        
        # Showdown using proper hand evaluation
        result = _evaluate_showdown(hero_hole, villain_hole, board)
