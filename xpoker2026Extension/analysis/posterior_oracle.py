...
class StrategyAwarePosterior:
    ...
    def _compute_hand_likelihood(
        ...
        for action_record in opponent_actions:
            ...
            if action_str in ("FOLD", "Folding"):
                ...
                # Unknown action type — use a uniform floor so it doesn't
                # silently bias the posterior by being skipped entirely
                likelihood *= (1.0 / 3.0)
                continue
            ...
            if "state" in action_record and action_record["state"]:
                ...
                # Create state from action record with available info
                state = PublicState(
                    street=action_record.get("street", "PREFLOP"),
                    board=action_record.get("board", []),
                    pot=action_record.get("pot", 0),
                    bet_to_call=action_record.get("bet_to_call", 0),
                    num_raises_this_street=action_record.get("num_raises", 0),
    ...
    def compute_for_hand(
        ...
        alt_key = f"{opponent_hand[1]}{opponent_hand[0]}"
        return posterior.get(hand_key, posterior.get(alt_key, 0.0))
...
def extract_opponent_actions(history: list[dict], hero_index: int = 0) -> list[dict]:
    ...
    for event in history:
        ...
        if event_type in ("DEAL_BOARD",):
            cards = event.get("cards", [])
            current_board = current_board + cards
            num_raises_this_street = 0
            max_bet_this_street = 0
            player_bets.clear()
        ...
        # Compute what faced this player BEFORE their action
        facing_bet = 0
        if player is not None:
            facing_bet = max(0, max_bet_this_street - player_bets.get(player, 0))

        # Update running state AFTER capturing facing_bet
        if amount is not None:
            running_pot += amount
        if player is not None and amount is not None:
            player_bets[player] = player_bets.get(player, 0) + amount
            max_bet_this_street = max(max_bet_this_street, player_bets[player])

        if event_type in ("BET", "RAISE", "CompletionBettingOrRaisingTo"):
        ...
        opponent_actions.append({
            ...
            "pot": running_pot,
            "bet_to_call": facing_bet,
            "num_raises": num_raises_this_street,
        })
