...
def get_update_type(prev_record: dict, curr_record: dict) -> str:
    ...
    if curr_history_len > prev_history_len:
        ...
        if curr_history_len > 0:
            ...
            hero_idx = curr_record.get("player_to_act", 0)
            has_opponent_action = False
            has_hero_action = False
            for event in new_events:
                player = event.get("player")
                action = event.get("event") or event.get("op", "")
                if player is not None and action in ["BET", "RAISE", "CALL", "CHECK", "FOLD"]:
                    if player != hero_idx:
                        has_opponent_action = True
                    else:
                        has_hero_action = True
            if has_opponent_action:
                return "ACTION"
            if has_hero_action:
                return "SELF_ACTION"
...
def compute_update_metrics(
    ...
    for i in range(len(BUCKET_NAMES)):
        ...
        both_near_zero = abs(llm_d) <= 0.001 and abs(oracle_d) <= 0.001
        same_direction = (llm_d > 0.001 and oracle_d > 0.001) or (llm_d < -0.001 and oracle_d < -0.001)
        if both_near_zero or same_direction:
...
def compute_summary(df: pd.DataFrame) -> dict:
    ...
    for update_type in ["CARD_REVEAL", "ACTION", "SELF_ACTION", "UNKNOWN"]:
