...
class PokerKitEnv:
    ...
    def reset_from_manifest(self, manifest: HandManifest) -> Obs:
        """
        Reset the environment to the same deal state as a recorded hand.
        
        This restores seed, hole cards, and board cards so the deal is
        identical.  Actions from the manifest are NOT replayed — the
        caller must step through them separately.
    ...
    def step(self, action: Action) -> tuple[Obs, float, bool, dict]:
        ...
        # Capture pre-action state for tracking (applied only after success)
        acting_player = self.current_player()
        pre_street = self._get_street_name()
        is_raise = action.type == ActionType.BET_OR_RAISE

        # Apply the action first so tracking only updates on success
        ...
        if pre_street != self._last_street:
            self._raises_this_street = 0
            self._last_street = pre_street
    ...
    def get_bet_to_call(self) -> int:
        ...
        # Get current bets, padded to num_players
        raw_bets = list(self.state.bets) if self.state.bets else []
        current_bets = raw_bets + [0] * (self.num_players - len(raw_bets))
    ...
    def get_button_position(self) -> int:
        ...
        For multi-way, this returns 0 by convention matching PokerKit's
        default seating where player 0 is the dealer.

        Returns:
            Player index of the button
        """
        if self.state and hasattr(self.state, "dealer_index"):
            return self.state.dealer_index
        return 0

    def get_player_position(self, player_index: int) -> str:
        """
        Get position name for a player relative to the current dealer.
        ...
        """
        btn = self.get_button_position()
        if self.num_players == 2:
            return "BTN/SB" if player_index == btn else "BB"

        positions = ["BTN", "SB", "BB", "UTG", "MP", "CO"]
        relative_seat = (player_index - btn) % self.num_players
        if relative_seat < len(positions):
            return positions[relative_seat]
        return f"P{relative_seat}"
    ...
    def get_contributions(self) -> tuple[list[int], list[int]]:
        ...
        # Current round contributions (current bets), padded to num_players
        raw_bets = list(self.state.bets) if self.state.bets else []
        contrib_round = raw_bets + [0] * (self.num_players - len(raw_bets))

        # Total contributions = initial stack - current stack
        # (current_stack already reflects all bets placed including the current round)
        contrib_total = []
        for i in range(self.num_players):
            current_stack = self.state.stacks[i]
            total = self.initial_stacks[i] - current_stack
    ...
    def _get_street_name(self) -> str:
        ...
        if board_count == 0:
            ...
            import warnings
            warnings.warn(
                f"Unexpected board card count: {board_count}. "
                f"Defaulting to PREFLOP.",
                UserWarning,
            )
            return "PREFLOP"
    ...
    def _get_street_index(self) -> int:
        ...
        if board_count == 0:
            ...
            import warnings
            warnings.warn(
                f"Unexpected board_count={board_count} in _get_street_index, "
                "returning 0 (PREFLOP) to stay consistent with _get_street_name"
            )
            return 0
