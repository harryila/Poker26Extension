...
def process_records(records: list[dict]) -> pd.DataFrame:
    ...
    for record in records:
        ...
        if np.isnan(js_cardonly) or np.isnan(js_strategyaware):
            continue
        
        # Get opponent's last action
        history = record.get("obs", {}).get("history", [])
        opp_action = get_opponent_last_action(history, record["player_to_act"])
        ...
        belief_sum_raw = record.get("belief_metadata", {}).get("prob_sum")
