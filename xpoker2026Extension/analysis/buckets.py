...
# Populate BUCKETS_7 dynamic entries ("suited" and "trash")
def _populate_scheme(scheme: dict[str, list[str]], bucket_names: list[str]) -> None:
    """Fill empty buckets in an alternative scheme with remaining patterns."""
    assigned = set()
    for name, patterns in scheme.items():
        assigned.update(patterns)
    remaining = _all_patterns - assigned
    # "suited" bucket gets all unassigned suited hands
    if "suited" in scheme and not scheme["suited"]:
        scheme["suited"] = sorted(p for p in remaining if p.endswith("s"))
        remaining -= set(scheme["suited"])
    # "trash" bucket gets everything else
    if "trash" in scheme and not scheme["trash"]:
        scheme["trash"] = sorted(remaining)

_populate_scheme(BUCKETS_7, BUCKET_NAMES_7)
_populate_scheme(BUCKETS_30, BUCKET_NAMES_30)
