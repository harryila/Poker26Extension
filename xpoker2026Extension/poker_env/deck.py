...
# Standard 52-card deck in ascending order (for deck generation/iteration).
# NOTE: analysis/buckets.py uses RANKS = "AKQJT98765432" (descending) for hand
# classification where lower index = higher rank. Both orderings are intentional.
RANKS = "23456789TJQKA"
...
class DeterministicDeck:
    ...
    def set_explicit_holes(self, hole_cards: list[str | None]) -> None:
        ...
        Raises:
            ValueError: If the same card appears in multiple players' holes.
        """
        ...
        # Validate each explicit hole is exactly 2 cards
        for idx, hole in enumerate(self._explicit_holes):
            if hole:
                cards = parse_cards(hole)
                if len(cards) != 2:
                    raise ValueError(
                        f"Explicit hole cards for player {idx} must be exactly "
                        f"2 cards, got {len(cards)}: '{hole}'"
                    )
        ...
        for hole in self._explicit_holes:
            if hole:
                all_explicit.extend(parse_cards(hole))
        if len(all_explicit) != len(set(all_explicit)):
            raise ValueError(
                f"Duplicate cards in explicit hole cards: {all_explicit}"
            )
    ...
    def set_explicit_board(self, board: str) -> None:
        ...
        """
        if board and self._explicit_holes:
            board_cards = set(parse_cards(board))
            hole_cards = set()
            for hole in self._explicit_holes:
                if hole:
                    hole_cards.update(parse_cards(hole))
            overlap = board_cards & hole_cards
            if overlap:
                raise ValueError(
                    f"Board cards overlap with hole cards: {overlap}"
                )
    ...
    def get_hole_cards(self, player_index: int) -> str:
        ...
        if len(self.cards) < 2:
            raise IndexError(
                f"Not enough cards in deck to deal hole cards "
                f"(need 2, have {len(self.cards)})"
            )
    ...
    def get_board_cards(self, count: int, current_board_len: int = 0) -> str:
        ...
        if self._explicit_board:
            ...
            if end <= len(explicit_cards):
                ...
                import warnings
                warnings.warn(
                    f"Explicit board has only {len(explicit_cards)} cards "
                    f"but {end} needed (start={start}, count={count}). "
                    f"Falling through to random deal.",
                    UserWarning,
                )
        ...
        if len(self.cards) < count:
            raise IndexError(
                f"Not enough cards in deck to deal board cards "
                f"(need {count}, have {len(self.cards)})"
            )
...
def parse_cards(card_string: str) -> list[str]:
    ...
    if len(card_string) % 2 != 0:
        import warnings
        warnings.warn(
            f"Card string '{card_string}' has odd length ({len(card_string)}); "
            f"trailing character will be ignored.",
            UserWarning,
        )
    cards = []
    i = 0
