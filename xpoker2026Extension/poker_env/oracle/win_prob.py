...
from poker_env.deck import FULL_DECK
...
class EquityOracle:
    ...
    def compute(
        ...
        if len(board) >= 5:
            if len(board) > 5:
                import warnings
                warnings.warn(
                    f"Board has {len(board)} cards (expected <=5), using first 5"
                )
    ...
    def _compute_monte_carlo(
        ...
        if len(deck) < cards_needed:
            return {"equity_win": 0.0, "equity_tie": 0.0, "equity_lose": 0.0}

        # Check count before materializing the full list
        from math import comb as _comb
        num_possible = _comb(len(deck), cards_needed)
