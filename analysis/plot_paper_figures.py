...
def figure1_pce_cdf(df: pd.DataFrame, summary: pd.DataFrame, output_dir: Path) -> None:
    ...
    # Sort values for CDF
    js_co = np.sort(df['js_cardonly'].values)
    js_sa = np.sort(df['js_strategyaware'].values)
    
    # CDF y-values
    cdf_y = np.arange(1, len(js_co) + 1) / len(js_co)
    
    # Plot CDFs
    ax.plot(js_co, cdf_y, color=COLORS['cardonly'], linewidth=2, 
            label=f'JS(LLM, CardOnly)', linestyle='-')
    ax.plot(js_sa, cdf_y, color=COLORS['strategyaware'], linewidth=2,
    ...
    # Add vertical dashed line showing the gap
...
def figure2_baserate_neglect(df: pd.DataFrame, summary: pd.DataFrame, output_dir: Path) -> None:
    ...
    if len(agg_rows) > 0 and len(pas_rows) > 0:
        llm_trash = [
...
def figure3_update_scatter(update_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    # Panel A: Card Reveals
    ax = axes[0]
    ax.scatter(card_df['oracle_magnitude'], card_df['llm_magnitude'],
               alpha=0.5, s=30, color=COLORS['card_reveal'], marker='o')
    
    # Diagonal reference line (perfect calibration)
    max_val = max(card_df['oracle_magnitude'].max(), card_df['llm_magnitude'].max()) * 1.1
    ax.plot([0, max_val], [0, max_val], 'k--', alpha=0.5, label='Perfect calibration (y=x)')
    
    # Regression line
    slope, intercept, r, p, se = stats.linregress(card_df['oracle_magnitude'], card_df['llm_magnitude'])
    x_line = np.array([0, card_df['oracle_magnitude'].max()])
    ...
    ax.set_title(f'(A) Card Reveals (N={len(card_df)})')
    ...
    ax.text(0.95, 0.05, f'r = {r:.3f}\nLLM updates {card_df["llm_magnitude"].mean() / card_df["oracle_magnitude"].mean():.1f}x more',
            transform=ax.transAxes, fontsize=10, ha='right', va='bottom',
            bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    
    # Panel B: Actions
    ax = axes[1]
    ax.scatter(action_df['oracle_magnitude'], action_df['llm_magnitude'],
               alpha=0.5, s=30, color=COLORS['action'], marker='s')
    
    # Diagonal reference line
    max_val = max(action_df['oracle_magnitude'].max(), action_df['llm_magnitude'].max()) * 1.1
    ax.plot([0, max_val], [0, max_val], 'k--', alpha=0.5, label='Perfect calibration (y=x)')
    
    # Regression line
    slope, intercept, r, p, se = stats.linregress(action_df['oracle_magnitude'], action_df['llm_magnitude'])
    x_line = np.array([0, action_df['oracle_magnitude'].max()])
    ax.plot(x_line, slope * x_line + intercept, 'r-', alpha=0.7, linewidth=2, label=f'Actual (r={r:.3f})')
    
    ax.set_xlabel('Oracle Update Magnitude')
    ax.set_ylabel('LLM Update Magnitude')
    ax.set_title(f'(B) Opponent Actions (N={len(action_df)})')
    ax.legend(loc='upper left', fontsize=9)
    ax.set_xlim(0, None)
    ax.set_ylim(0, None)
    ax.grid(True, alpha=0.3)
    ...
    # Annotate correlation
    ax.text(0.95, 0.05, f'r = {r:.3f}\nLLM updates {action_df["llm_magnitude"].mean() / action_df["oracle_magnitude"].mean():.1f}x more',
            transform=ax.transAxes, fontsize=10, ha='right', va='bottom',
            bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
